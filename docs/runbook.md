...
Last reviewed: 2026-06-13 (ADR 0039 fifteenth audit landed, the first audit pass over the ADR 0028-0038 Vertex MCP capability wave; next audit slot is ADR 0040).
...
5. `docs/adr/README.md`, then the most recent ADRs (`0037`, `0038`, `0039`) for the cross-cutting decisions in force.
...
|---|---|---|
| Read-vs-CAS / read-vs-listing expiry boundary | A live key disappears or a CAS gate flips at the exact expiry instant | `BL-157` (DynamoDB CAS), `BL-168` (SQLite sweep), `BL-177` (DynamoDB read), `BL-188` (in-tree listing), `BL-260` (S3 list-then-HEAD lazy-delete on the read path) |
| Non-atomic / compute-outside-lock multi-step mutation | A value computed before the transaction lock, or a multi-key mutation split across round trips, diverges from the committed snapshot on a concurrent write or a crash | `BL-259` (SQLite TTL expiry clock read before `BEGIN IMMEDIATE` in `mset` / `compare_and_set` / `write_versioned` / `transact`), `BL-262` (`BoundedRedisStore.write` value SET + capacity-index ZADD not in one `MULTI`/`EXEC`) |
| Path-traversal / symlink dereference on install | A pre-existing link or a crafted local mirror at `dest/<name>` | `BL-169` (`LocalSkillSource`), `BL-172` (network sources), `BL-190` (`LocalSkillSource` clean-error), `BL-254` (`LocalSkillSource` hardlink refusal) |
...
|---|---|---|---|
| `README.md` | stable | Status paragraph cites the latest ADR (today `0039`, the fifteenth audit, plus the ADR 0028-0038 Vertex MCP capability wave, the ADR 0027 deferred-resume, and the earlier audit-wave enumeration back through ADR 0017); the capability bullets match the present `harness/` / `memory/` / `skills/` / `evaluation/` exports (including `approval_mode="deferred"`, the `model_settings` pass-through + cache surfacing, `TransactionalMemoryStore`, `MemoryCompactor` / `TruncatingSummarizer`, `TieredMemoryStore`, `BoundedSweepableStore` and `BoundedRedisStore` / `BoundedDynamoDBStore` / `BoundedS3Store`). Tracked gap: the Capabilities bullets do not yet enumerate the ADR 0028-0038 Vertex-wave surface (hybrid retrieval / `query_hybrid` / `fuse_rrf`, `AuthorityTier` + `TierClassifier`, `RunRecord.degraded` + grounding, the `BitemporalMemoryStore`, the operational-memory `Journal` + `context_pack`, `FallbackChain`, `require_fresh` + `Refusal`, the `EvidenceHook`); add them when the Capabilities list is next revised; the install line lists every optional extra (`redis`, `aws`, `crypto`, `otel`, `anthropic`, `openai`); the seven-dispatcher count (`BL-160` errata) | A new ADR, a new top-level capability, a new extra |
| `CLAUDE.md` | stable | The ADR enumeration (today `0007`-`0039`); the layout block matches `ls`; the `evaluation/` line ships; the additive-to-L1 rule wording is the current canonical phrasing | A new ADR, a new top-level component, a layout change |
| `STATUS.md` | living | Last-reviewed date is today; the phase-tracking table cites the latest ADR; the document-maturity table covers every `.md` in the tree (the table mentions `0001-0039`); the L3-open row is the current `[pending]` set | Every audit, every release rehearsal |
...
| `docs/adr/0001`-`0039` | stable, Accepted | Frozen; errata are recorded in the next ADR, not edited in place (`ADR 0009 -> 0010` errata template) | Never |
