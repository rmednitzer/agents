...
1. The expiry-boundary class. `BL-157`, `BL-168`, `BL-177`, `BL-188`. Three of the five adapters and the listing paths have been fixed pointwise; a "boundary helper" (`_live(now, expires_at)` in `memory/_audit.py` or `memory/validators.py`) would make every read / CAS / scan / sweep call the same predicate, by construction. Cost: a non-trivial diff across five adapters. Benefit: the next adapter cannot reintroduce the boundary class. Open question: whether the durable adapters' SQL / DSL conditions (`exp >= :now`) compose with a Python predicate; if not, the helper is documentation, not code.
...
4. The `EncryptedStore` legacy migration. `BL-181` ships an authenticated legacy fallback under the current key only; a multi-key legacy fallback would let a versioned provider also read a non-current key's legacy ciphertext, eliminating the "re-encrypt through the old store first" migration step. Cost: a per-version legacy attempt loop, bounded; benefit: a smoother rotation path. Open question: whether the AES-GCM authentication tag is strong enough on its own that "try each historical key" is acceptable.
