...
- `BL-135` [in-progress] [L] Memory compaction, summarisation, and tiering (hot to cold), and a size or LRU bound on the sweeper, not age only. Delivered: the `BoundedSweepableStore` extension Protocol on `InMemoryStore` and `TTLSweeper(max_keys=...)` (`BL-212`); `BoundedSweepableStore` on the durable single-host `SQLiteStore` adapter (`BL-213`). Remaining: compaction / summarisation / tiering (the long-horizon context-engineering half, S2), plus the `BoundedSweepableStore` reference on the remaining durable adapters (`RedisStore` insertion-ordered sorted-set indexer, `DynamoDBStore` scan + delete batch, `S3Store` LIST + DELETE). Each remaining adapter needs an auxiliary index or scan-based approach because no native insertion-order column exists the way SQLite's rowid does. Long-horizon workloads still grow unbounded by compaction (S2: context compaction). (ADR 0004, ADR 0008)
