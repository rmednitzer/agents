...
- `BL-135` [in-progress] [L] Memory compaction, summarisation, and tiering (hot to cold), and a size or LRU bound on the sweeper, not age only. Delivered (`BL-212`): the size-bound half via the `BoundedSweepableStore` extension Protocol on `InMemoryStore` and `TTLSweeper(max_keys=...)`. Remaining: compaction / summarisation / tiering (the long-horizon context-engineering half, S2), plus the `BoundedSweepableStore` reference on the durable adapters (`SQLiteStore` / `RedisStore` / `DynamoDBStore` / `S3Store`) following the `BL-124` -> `BL-180` Protocol-plus-reference-first cadence. Long-horizon workloads still grow unbounded by compaction (S2: context compaction). (ADR 0004, ADR 0008)
