...
clean, `BL-240` / `BL-241` proposed for maintainer decision), and
`0026` (`BL-132` / `BL-171`: prompt caching on the runtime adapter,
unblocked by pydantic-ai 1.106; the opt-in `model_settings`
pass-through, opaque to the harness like `model`, and
`BudgetTracker.consume_cache_tokens` surfacing of cache hit/creation
counts, not charged to `max_tokens` and outside `snapshot()`; live
cache-hit validation coupled to `BL-120`), and `0027` (`BL-114`:
deferred non-replay approval resume, also unblocked by pydantic-ai
1.106; the opt-in `approval_mode="deferred"` rides
`DeferredToolRequests` / `DeferredToolResults`, the paused leg's
message history travels in the new optional
`ResumableState.runtime_state`, prior tool calls run exactly once on
resume, the BL-193 (tool, arguments) binding is re-verified at
execution, denial is model-visible via `ToolDenied`, and replay
stays the byte-identical default), the Vertex MCP analysis wave
`0028`-`0038` (`BL-242`-`BL-253`: hybrid retrieval fusion +
decay-ranked demotion, graduated authority tiers, DEGRADED
disposition + grounding, the approval-context payload, the
bitemporal fact store, two-step parameter restatement, the
operational-memory journal, the fallback chain, read-side
freshness, session rehydration, and the evidence-capture hook),
and `0039` (fifteenth audit; `BL-254`-`BL-262`: `LocalSkillSource`
hardlink refusal, the `DriftMonitor.record` and `query_hybrid`
reranker non-finite guards generalising the BL-159 / BL-221 /
BL-231 NaN class to two run-time value boundaries, the `fuse_rrf`
distinct-rank fix so a duplicate id no longer penalises the RRF
score of later unique ids, the SQLite TTL-clock-after-`BEGIN
IMMEDIATE` ordering fix, the S3 list-vs-HEAD no-lazy-delete fix
generalising the BL-188 read-vs-listing boundary class, the
`BoundedRedisStore.write` set-plus-index atomicity fix, the
`grounding._grounded` short-circuit, and the evidence-hook
fail-safe ordering docstring).
