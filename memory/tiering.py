...
- ``write`` lands in hot first, then invalidates the cold copy (by
  ...
  window for keys that are rewritten and then expire hot.
...
- ``demote`` on a ``VersionedMemoryStore`` hot tier deletes the hot
  ...
  (``delete_versioned``); a concurrently rewritten key stays hot and
  is not counted, and the just-written cold copy is shadowed by the
  newer hot value until the next demotion or invalidation.
...
class TieredMemoryStore:
    ...
    def __init__(
        ...
        *,
        ...
        # legacy sentinel 0 at ranking time.
    ...
    async def read(self, key: str) -> bytes | None:
        ...
        if self._promote_on_read:
            hot = self._hot
            if isinstance(hot, CASMemoryStore):
                ...
                await hot.compare_and_set(key, None, value)
            else:
                await hot.write(key, value)
    ...
    async def write(
        ...
        *,
        ...
        # copy that the newer hot value shadows on read.
    ...
    async def demote(
        ...
        *,
        ...
        version-gated: a key rewritten between read and delete stays
        hot and is not counted (the newer hot value shadows the stale
        cold copy). Returns the number of keys actually moved. Keys are
        validated up front (all-or-nothing, the BatchMemoryStore
        convention); absent and expired keys are skipped.
        """
        ...
        hot = self._hot
        moved = 0
        if isinstance(hot, VersionedMemoryStore):
            for key in ordered:
    ...
    async def demote_to_capacity(
        ...
        *,
        ...
        the live hot keyspace is at or under the cap. ``max_hot_keys``
        must be positive (zero would empty the hot tier; call
        ``demote`` with every key if that is the intent).
        """
        if max_hot_keys <= 0:
        ...
        self._order = {k: s for k, s in self._order.items() if k in live_set}
